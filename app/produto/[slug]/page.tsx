...
export async function generateMetadata({ params }: Props) {
  const p = getProduto(params.slug);
    ...
    <main className="min-h-screen bg-white">
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="space-y-3">
            {p.estrutura.map((mod, i) => (
              <div
                key={i}
                className="p-5 bg-white rounded-2xl border border-neutral-200 flex items-start gap-4"
              >
        ...
        <div className="max-w-4xl mx-auto px-6">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            {p.beneficios.map((b, i) => (
              <div
                key={i}
                className="p-5 rounded-2xl border border-neutral-200 flex items-start gap-3"
              >
          ...
          <div className="max-w-3xl mx-auto px-6">
            ...
            <div className="space-y-4">
              {p.bonus.map((b, i) => (
                <div
                  key={i}
                  className="p-5 bg-white rounded-2xl border border-brand-200 flex items-center gap-4"
                >
      ...
      <section className="py-16 bg-neutral-50">
        <div className="max-w-3xl mx-auto px-6 text-center">
          <p className="text-sm font-bold text-neutral-500 uppercase tracking-widest mb-3">
            Preview
          </p>
          <h3 className="text-2xl font-black mb-6">Baixe uma amostra do material</h3>
          <a
            href={`/api/gerar-pdf?slug=${p.slug}`}
        ...
        <div className="max-w-3xl mx-auto px-6">
          ...
          <div className="space-y-3">
            {p.faq.map((f, i) => (
              <details
                key={i}
                className="group bg-neutral-50 rounded-2xl p-6 cursor-pointer"
              >
